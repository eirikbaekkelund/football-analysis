...
def visualize_semantic_mask(
    image: Image.Image, seg_mask: mx.array, alpha: float = 0.5, color: tuple = (255, 0, 0)
) -> Image.Image:
    """
...
def main(prompt: str = "player in white", image_path: str = "test_image.jpg"):
    start = time.perf_counter()
    script_dir = os.path.dirname(os.path.abspath(__file__))
    bpe_path = os.path.join(script_dir, "assets", "bpe.txt.gz")
    checkpoint_path = os.path.join(script_dir, "assets", "model.safetensors")
    ...
    if not os.path.isabs(image_path):
        search_paths = [os.getcwd(), script_dir, os.path.join(script_dir, "assets"), os.path.dirname(script_dir)]
        found = find_file(image_path, search_paths)
        if found:
            image_path = found
        else:
            raise FileNotFoundError(f"Image '{image_path}' not found in search paths: {search_paths}")
    ...
    if "semantic_seg" in output:
        ...
        overlay_img = visualize_semantic_mask(image, seg_mask, alpha=0.5, color=(0, 255, 128))  # Green-ish overlay
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image", default="test_image.jpg", help="Path to the input image")
    parser.add_argument("--prompt", default="player in white", help="Text prompt for segmentation")
    args = parser.parse_args()
    main(prompt=args.prompt, image_path=args.image)
