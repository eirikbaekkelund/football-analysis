...
import mlx.nn as nn
from torchkick.sam_3._mlx.backbone.box_ops import box_cxcywh_to_xyxy
from torchkick.sam_3._mlx.backbone.grid_sample import grid_sample
from torchkick.sam_3._mlx.backbone.roi_align import roi_align
from torchkick.sam_3._mlx.backbone.utils import get_clones
...
def concat_padded_sequences(seq1, mask1, seq2, mask2, return_index: bool = False):
    ...
    concatenated_mask = mx.tile(mx.arange(max_length)[None], (batch_size, 1)) >= final_lengths[:, None]
    # (max_len, batch_size, hidden_size)
    concatenated_sequence = mx.zeros((max_length, batch_size, hidden_size), dtype=seq2.dtype)
...
class Prompt:
    ...
    def __init__(
        ...
        box_embeddings, box_labels, box_mask = self._init_box(box_embeddings, box_labels, box_mask, box_seq_len, bs)
    ...
    def _init_point(self, point_embeddings, point_labels, point_mask, point_seq_len, bs):
        """
    ...
    def _init_mask(self, mask_embeddings, mask_labels, mask_mask, mask_seq_len, bs):
    ...
    def append_boxes(self, boxes, labels, mask=None):
        ...
        self.box_labels, _ = concat_padded_sequences(self.box_labels[..., None], self.box_mask, labels[..., None], mask)
...
class SequenceGeometryEncoder(nn.Module):
    ...
    def _encode_masks(
        ...
        masks = masks + pos
        masks = masks.reshape(n_masks, bs, *masks.shape[1:]).flatten(-2)  # n_masks x bs x C x H*W
    ...
    def __call__(self, geo_prompt: Prompt, img_feats, img_sizes, img_pos_embeds=None):
        ...
        seq_first_img_pos_embeds = (
            img_pos_embeds[-1] if img_pos_embeds is not None else mx.zeros_like(seq_first_img_feats)
        )
