...
from torchkick.sam_3._mlx.backbone.utils import MLP
    ...
    def __call__(self, obj_queries, pixel_embed):
        if len(obj_queries.shape) == 3:
            if pixel_embed.ndim == 3:
                ...
                mask_preds = mx.einsum("bqc,chw->bqhw", self.mask_embed(obj_queries), pixel_embed)
            else:
                mask_preds = mx.einsum("bqc,bchw->bqhw", self.mask_embed(obj_queries), pixel_embed)
        else:
            # Assumed to have aux masks
            if pixel_embed.ndim == 3:
                # batch size was omitted
                mask_preds = mx.einsum("lbqc,chw->lbqhw", self.mask_embed(obj_queries), pixel_embed)
            else:
                mask_preds = mx.einsum("lbqc,bchw->lbqhw", self.mask_embed(obj_queries), pixel_embed)
    ...
    def __call__(self, backbone_feats: List[mx.array]):
        ...
        for layer_idx, bb_feat in enumerate(fpn_feats[::-1]):
            ...
            upsample_fn = nn.Upsample(
                scale_factor=(scale_h, scale_w), mode=self.interpolation_mode, align_corners=False
            )
