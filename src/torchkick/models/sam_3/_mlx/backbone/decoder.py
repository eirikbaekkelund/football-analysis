...
from torchkick.sam_3._mlx.backbone.utils import (
    get_activation_fn,
    get_clones,
        ...
        **kwargs,  # additional kwargs for compatibility
        ...
        if self.self_attn is not None:
            ...
            if presence_token is not None:
                ...
                tgt_query_pos_o2o = mx.concat([mx.zeros_like(presence_token), tgt_query_pos], axis=0)
                tgt_query_pos = mx.concat([mx.zeros_like(presence_token), tgt_query_pos], axis=0)
        ...
        if presence_token is not None:
            ...
            cross_attn_mask = mx.concat([presence_token_mask, cross_attn_mask], axis=1)  # (bs*nheads, 1+nq, hw)
...
class TransformerDecoder(nn.Module):
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        ...
        if self.boxRPB in ["log", "both"]:
            ...
            deltas_x_log = mx.sign(deltas_x_log) * mx.log2(mx.abs(deltas_x_log) + 1.0) / np.log2(8)

            deltas_y_log = deltas_y * 8  # normalize to -8, 8
            deltas_y_log = mx.sign(deltas_y_log) * mx.log2(mx.abs(deltas_y_log) + 1.0) / np.log2(8)
            if self.boxRPB == "log":
    ...
    def __call__(
        ...
        if apply_dac:
            assert (tgt.shape[0] == self.num_queries) or (
                self.use_instance_query and (tgt.shape[0] == self.instance_query_embed.weight.shape[0])
            )
            ...
            if reference_boxes is not None:
                assert (reference_boxes.shape[0] == self.num_queries) or (
                    self.use_instance_query and (reference_boxes.shape[0] == self.instance_query_embed.weight.shape[0])
                )
        ...
        if self.box_refine:
            if reference_boxes is None:
                ...
                reference_boxes = (
                    mx.tile(reference_boxes, (2, bs, 1)) if apply_dac else mx.tile(reference_boxes, (1, bs, 1))
                )
        ...
        for layer_idx, layer in enumerate(self.layers):
            reference_points_input = (
                reference_boxes[:, :, None] * mx.concat([valid_ratios, valid_ratios], -1)[None, :]
            )  # nq, bs, nlevel, 4
