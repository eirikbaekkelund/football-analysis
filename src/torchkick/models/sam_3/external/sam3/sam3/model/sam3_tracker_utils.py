...
# Prefer Triton EDT when available (fast GPU). Fall back to CPU EDT
# implemented with SciPy/OpenCV to support macOS/dev environments.
try:
    from sam3.model.edt import edt_triton  # type: ignore
except Exception:
    import warnings

    def edt_triton(data: torch.Tensor):
        """CPU fallback for EDT. Accepts a torch tensor of shape (B, H, W)
        and returns a float tensor of same shape containing distances.
        Tries SciPy's distance_transform_edt, then OpenCV as a last resort.
        """
        ...
        try:
            ...
            for b in range(B):
                out_list.append(ndimage.distance_transform_edt(arr[b]))
            out = np.stack(out_list, axis=0).astype(np.float32)
            return torch.from_numpy(out).to(data.device)
        except Exception:
            warnings.warn("scipy not available or failed; attempting OpenCV EDT fallback")
        # Fallback: OpenCV
        try:
            import cv2

            for b in range(B):
                img = arr[b].astype(np.uint8) * 255
                out_list.append(cv2.distanceTransform(img, cv2.DIST_L2, 0))
            out = np.stack(out_list, axis=0).astype(np.float32)
            return torch.from_numpy(out).to(data.device)
        except Exception as e:
            raise RuntimeError(
                "EDT fallback failed: install scipy or opencv-python, or run on a machine with Triton." + str(e)
            )
