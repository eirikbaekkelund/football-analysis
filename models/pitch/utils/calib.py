...
from models.pitch.utils.optimize import (
    vector_to_mtx,
...
line_world_coords_3D = [
    [[x1 - 52.5, y1 - 34, z1], [x2 - 52.5, y2 - 34, z2]] for [[x1, y1, z1], [x2, y2, z2]] in line_world_coords_3D
]
...
def pan_tilt_roll_to_orientation(pan, tilt, roll):
    ...
    """
    Rpan = np.array([[np.cos(pan), -np.sin(pan), 0], [np.sin(pan), np.cos(pan), 0], [0, 0, 1]])
    Rroll = np.array([[np.cos(roll), -np.sin(roll), 0], [np.sin(roll), np.cos(roll), 0], [0, 0, 1]])
    Rtilt = np.array([[1, 0, 0], [0, np.cos(tilt), -np.sin(tilt)], [0, np.sin(tilt), np.cos(tilt)]])
...
class FramebyFrameCalib:
    ...
    def reproj_err(self, obj_pts, img_pts):
        if self.calibration is not None:
            ...
            Q = np.array([[x_focal_length, 0, x_principal_point], [0, y_focal_length, y_principal_point], [0, 0, 1]])
            P = Q @ (rotation @ It)

            err, n = 0, 0
            for i in range(len(obj_pts)):
                proj_point = P @ np.array([obj_pts[i][0], obj_pts[i][1], obj_pts[i][2], 1.0])
    ...
    def reproj_err_ground(self, obj_pts, img_pts):
        if self.homography is not None:
            err, n = 0, 0
            for i in range(len(obj_pts)):
                proj_point = self.homography @ np.array([obj_pts[i][0], obj_pts[i][1], 1.0])
                proj_point /= proj_point[-1]
                err_point = img_pts[i] - proj_point[:2]
                err += np.sum(err_point**2)
                n += 1
    ...
    def get_cam_params(self, mode='full', use_ransac=0, refine=False, refine_w_lines=False):
        ...
        flags = flags | cv2.CALIB_FIX_TANGENT_DIST | cv2.CALIB_FIX_S1_S2_S3_S4 | cv2.CALIB_FIX_TAUX_TAUY
        flags = (
            flags
            | cv2.CALIB_FIX_K1
            | cv2.CALIB_FIX_K2
            ...
            if refine:
                if not np.isnan(rep_err):
                    ...
                    rvecs, tvecs = cv2.solvePnPRefineLM(
                        ...
                        rvec,
                        tvec,
                        (cv2.TERM_CRITERIA_MAX_ITER + cv2.TERM_CRITERIA_EPS, 20000, 1e-5),
                    )
    ...
    def get_homography_from_ground_plane(self, use_ransac=5.0, inverse=False, refine_lines=False):
        ...
        for mode in ['full', 'ground_plane', 'main']:
            for use_ransac in [0, 5, 10, 15, 25, 50]:
                cam_params, ret = self.get_cam_params(
                    mode=mode, use_ransac=use_ransac, refine=refine, refine_w_lines=refine_lines
                )
                if ret:
                    result_dict = {
                        'mode': mode,
